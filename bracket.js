  ...
  const roundsHTML = rounds.map((roundName, ri) => {
    ...
    return `
      <div class="bracket-round" data-round="${roundName}" data-round-idx="${ri}">
        ${matches.map(m => renderMatch(m, catLabel, isAdmin, editMode)).join('')}
      </div>
    `;
  }).join('');
...
function layoutBracket(container) {
  ...
  // CUSTOM POSITIONS: lê do STATE se admin moveu matches livremente.
  // Aplica como TRANSFORM (deslocamento sobre a posição natural), preservando
  // toda a lógica de colunas. Fallback total se STATE não acessível.
  let _customPositions = {};
  try {
    ...
    if (_catId && typeof STATE !== 'undefined' && STATE.brackets && STATE.brackets[_catId]) {
      _customPositions = STATE.brackets[_catId].customPositions || {};
    }
  } catch (e) { /* fallback silencioso */ }

  // Aplica transform para cada match com customPosition
  if (Object.keys(_customPositions).length > 0) {
    const _innerRect = inner.getBoundingClientRect();
    let _maxCustomY = 0, _maxCustomX = 0;
    inner.querySelectorAll('.bk-match').forEach(m => {
      const id = m.dataset.matchId;
      const cp = _customPositions[id];
      if (cp && typeof cp.x === 'number' && typeof cp.y === 'number') {
        const cardRect = m.getBoundingClientRect();
        const curX = cardRect.left - _innerRect.left;
        const curY = cardRect.top - _innerRect.top;
        const dx = cp.x - curX;
        const dy = cp.y - curY;
        m.style.transform = `translate(${dx}px, ${dy}px)`;
        m.style.zIndex = '5';
        m.classList.add('has-custom-pos');
        if (cp.y + matchHeight > _maxCustomY) _maxCustomY = cp.y + matchHeight;
        if (cp.x > _maxCustomX) _maxCustomX = cp.x;
      } else {
        m.style.transform = '';
        m.classList.remove('has-custom-pos');
      }
    });
    if (_maxCustomY + 80 > maxRoundHeight) {
      inner.style.minHeight = `${_maxCustomY + 80}px`;
    }
  } else {
    // Sem customs: limpa transforms eventuais
    inner.querySelectorAll('.bk-match.has-custom-pos').forEach(m => {
      m.style.transform = '';
      m.classList.remove('has-custom-pos');
    });
  }
...
/* Helper exposto: re-aplicar layout completo (usado após drag pra atualizar custom transforms) */
window.tpRelayoutBracket = function(container) {
  layoutBracket(container);
};
